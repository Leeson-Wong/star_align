...
public:
	...
	constexpr friend bool operator<(CStarDist const& lhs, CStarDist const& rhs) noexcept
	{
		if (lhs.m_Star1 != rhs.m_Star1) return lhs.m_Star1 < rhs.m_Star1;
...
//	void AdjustVoting(const VOTINGPAIRVECTOR& vInVotingPairs, VOTINGPAIRVECTOR& vOutVotingPairs, int lNrTgtStars);
	...
	bool ComputeTransformation(const VOTINGPAIRVECTOR& vVotingPairs, CBilinearParameters& BilinearParameters, const TRANSFORMATIONTYPE TType);
