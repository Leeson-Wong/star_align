...
bool CMatchingStars::ComputeCoordinateTransformation(CBilinearParameters& BilinearParameters)
{
	...
	{
		fprintf(stderr, "[DSS] ComputeCoordinateTransformation: refStars=%zu tgtStars=%zu lockCorners=%s\n",
			m_vRefStars.size(), m_vTgtStars.size(), AreCornersLocked() ? "true" : "false");
		bool bResult = ComputeLargeTriangleTransformation(BilinearParameters);
		if (!bResult)
		{
			fprintf(stderr, "[DSS] LargeTriangle FAILED, trying MatchingTriangle\n");
			bResult = ComputeMatchingTriangleTransformation(BilinearParameters);
		}
		if (bResult)
		{
			fprintf(stderr, "[DSS] Transform params: a0=%.10f a1=%.10f a2=%.10f a3=%.10f b0=%.10f b1=%.10f b2=%.10f b3=%.10f\n",
				BilinearParameters.a0, BilinearParameters.a1, BilinearParameters.a2, BilinearParameters.a3,
				BilinearParameters.b0, BilinearParameters.b1, BilinearParameters.b2, BilinearParameters.b3);
		}
		return bResult;
	}
	fprintf(stderr, "[DSS] ComputeCoordinateTransformation: not enough stars (ref=%zu tgt=%zu)\n",
		m_vRefStars.size(), m_vTgtStars.size());
	return false;
}
